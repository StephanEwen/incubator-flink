 ...
 */
class S3RecoverableSerializer implements SimpleVersionedSerializer<S3Recoverable> {
	...
	public byte[] serialize(S3Recoverable obj) throws IOException {
		...
		final byte[] keyBytes = obj.key().getBytes(CHARSET);
		...
		final String lastObjectKey = obj.lastPartObject();
		...
		final byte[] targetBytes = new byte[
						...
						12 + // length fields
						4 + keyBytes.length +
						4 + uploadIdBytes.length +
						4 + partEtagBytes +
						4 + (lastPartBytes == null ? 0 : lastPartBytes.length)
				];
		...
		bb.putInt(obj.lastPartObjectLength());
	...
	private static S3Recoverable deserializeV1(byte[] serialized) throws IOException {
		...
		final int lastPartLength = bb.getInt();
