...
public class AscendingTimestampsWatermarks<T> implements WatermarkGenerator<T> {
	...
	public void onEvent(T event, long eventTimestamp, WatermarkOutput output) {
		// we use Math.max() to guard against cases of outliers that violate monotony
		lastTimestamp = Math.max(lastTimestamp, eventTimestamp);
	}

	@Override
	public void onPeriodicEmit(WatermarkOutput output) {
		output.emitWatermark(new Watermark(lastTimestamp - 1));
	}
}
