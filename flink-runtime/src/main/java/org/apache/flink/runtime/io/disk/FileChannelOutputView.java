...
public class FileChannelOutputView extends AbstractPagedOutputView {
	...
	public FileChannelOutputView(BlockChannelWriter writer, MemoryAllocator allocator, int numBlocksToUse) throws IOException {
		super(allocator.getMemorySegmentSize(), 0);
		...
		if (allocator.getGuaranteedNumberOfSegments() < numBlocksToUse) {
			throw new IllegalArgumentException("The FileChannelOutputView cannot use more memory segments than its memory allocator has reserved.");
		}
		...
		try {
			for (int i = 0; i < numBlocksToUse; i++) {
				...
				if (next == null) {
					throw new IOException("Cannot acquire memory for FileChannelOutputView");
				}
				memory.add(next);
			...
			while (writer.getReturnQueue() != null);
			throw e;
		}
