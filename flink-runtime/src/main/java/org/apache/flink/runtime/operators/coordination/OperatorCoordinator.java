...
public interface OperatorCoordinator extends AutoCloseable {
	...
	/**
	 * Finishes the work of the OperatorCoordinator. This method is called when the job finishes successfully.
	 * Examples for this are global finalization or commit/publishing steps.
	 *
	 * <p>The method is considered part of the execution, so a failure in this method means a failure of the job.
	 * For cleanup operations, use {@link #close()} instead.
	 *
	 * @throws Exception Any exception thrown from this method causes a full job failure (and recovery).
	 */
	void finish() throws Exception;

	/**
	 * This method is called when the coordinator is disposed. This method should release currently
	 * held resources. Unlike the {@link #finish()} method, exceptions in this method do not cause
	 * the job to fail.
	 */
	@Override
	...
	interface Context {
		...
		void sendEvent(OperatorEvent evt, int targetSubtask) throws TaskNotRunningException;
