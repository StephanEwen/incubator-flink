...
final class ExecutionJobVertexCoordinatorContext implements OperatorCoordinator.Context {
	...
	@Override
	public void sendEvent(OperatorEvent evt, int targetSubtask) throws TaskNotRunningException {
		...
		final boolean sent = getTaskExecution(targetSubtask).sendOperatorEvent(operatorId, serializedEvent);
		if (!sent) {
