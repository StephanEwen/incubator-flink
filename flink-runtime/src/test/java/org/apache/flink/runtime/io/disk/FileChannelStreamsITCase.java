...
public class FileChannelStreamsITCase {
	...
	private TestMemoryAllocator memoryAllocator;

	// --------------------------------------------------------------------------------------------

	@Before
	public void beforeTest() {
		memoryAllocator = new TestMemoryAllocator(NUM_MEMORY_SEGMENTS, MEMORY_PAGE_SIZE);
	...
	public void afterTest() {
		...
		assertTrue("The memory has not been properly released", memoryAllocator.allSegmentsAvailable());
	}
	...
	public void testWriteReadSmallRecords() {
		try {
			...
			final FileChannelOutputView outView = new FileChannelOutputView(writer, memoryAllocator, NUM_MEMORY_SEGMENTS);
			...
			final FileChannelInputView inView = new FileChannelInputView(reader, memoryAllocator, NUM_MEMORY_SEGMENTS, outView.getBytesInLatestSegment());
			generator.reset();
	...
	public void testWriteReadOneBufferOnly() {
		try {
			...
			// create the writer output view
			final BlockChannelWriter writer = this.ioManager.createBlockChannelWriter(channel);
			final FileChannelOutputView outView = new FileChannelOutputView(writer, memoryAllocator, 1);
			...
			// create the reader input view
			final BlockChannelReader reader = ioManager.createBlockChannelReader(channel);
			final FileChannelInputView inView = new FileChannelInputView(reader, memoryAllocator, 1, outView.getBytesInLatestSegment());
			generator.reset();
